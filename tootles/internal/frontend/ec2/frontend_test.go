...
func TestFrontendNetworkUnknownMacOrLeaf(t *testing.T) {
	ptr := func(s string) *string { return &s }
	instance := data.Ec2Instance{
		Metadata: data.Metadata{
			Network: data.InstanceNetwork{
	...
	for _, endpoint := range []string{
		...
		router.ServeHTTP(w, r)
		if w.Code != http.StatusNotFound {
			t.Errorf("%s: expected 404, got %d (body=%q)", endpoint, w.Code, w.Body.String())
		}
	}
}

func TestFrontendNetworkReturns404WhenInstanceEndpointDisabled(t *testing.T) {
	ctrl := gomock.NewController(t)
	client := ec2.NewMockClient(ctrl)
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest("GET", "/tootles/instanceID/some-instance/2009-04-04/meta-data/network/interfaces/macs", nil)
	r.RemoteAddr = "10.10.10.10:0"
	router.ServeHTTP(w, r)

	if w.Code != http.StatusNotFound {
		t.Errorf("expected 404 for instance-id-scoped network route when disabled, got %d", w.Code)
	}
}
